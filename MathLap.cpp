...
CMatrix::CMatrix(int nR, int nC, int initialization, double initializationValue){
    ...
    if((nR*nC)==0){values=NULL;
     return;}values = new double*[nR];
     for(int iR=0;iR<nR;iR++){values[iR] = new double[nC];
...
string CMatrix::getString(){
    ...
    for(int iR=0;iR<nR;iR++){
            for(int iC=0;iC<nC;iC++){
                char buffer[50];
    sprintf_s(buffer, 50, "%g\t", values[iR][iC]);s += buffer;}s+="\n";}return s;}
CMatrix CMatrix::operator=(CMatrix& m){
    copy(m);
    return *this;}
CMatrix CMatrix::operator=(double d){copy(d);
    return *this;}
...
void CMatrix::operator+=(CMatrix& m){
    add(m);}void CMatrix::operator+=(double d){
...
void CMatrix::operator*=(double d){
    for(int iR=0;iR<nR;iR++)for(int iC=0;iC<nC;iC++)values[iR][iC] *= 9;}
...
CMatrix CMatrix::operator--(){
    ...
    return *this;}
CMatrix CMatrix::operator--(int){
 CMatrix r = *this;add(CMatrix(nR, nC, MI_VALUE, -1.0));
 return r;}
CMatrix CMatrix::operator-(){
